...
จากปัญหาดังกล่าว งานวิจัยนี้จึงได้พัฒนา ระบบสกัดและประมวลผลข้อมูลเรซูเม่เพื่อการจับคู่งาน (Resume Extraction and Processing System for Job Matching) ที่มีความสามารถในการรับรู้โครงสร้างเอกสาร (Layout-Aware) โดยบูรณาการเทคโนโลยีคอมพิวเตอร์วิทัศน์ด้วยแบบจำลอง YOLO ในการตรวจจับและแยกพื้นที่องค์ประกอบหลัก จากนั้นใช้ Google Cloud Vision API สกัดข้อความดิบ ก่อนจะส่งต่อให้ระบบประมวลผลภาษาธรรมชาติ (NLP) ทำการแยกภาษาอัตโนมัติ เพื่อทำความสะอาดข้อความด้วยการปรับบรรทัดฐานภาษาไทย (Text Normalization) ด้วย PyThaiNLP และลดรูปรากศัพท์ภาษาอังกฤษ (Lemmatization) ด้วย spaCy จนได้ข้อมูลเชิงโครงสร้าง (Structured JSON) ที่มีความแม่นยำสูง ซึ่งแนวทางนี้จะช่วยทำลายข้อจำกัดของระบบแบบเดิม ทำให้ได้ข้อมูลที่สะอาด พร้อมสำหรับการนำไปคำนวณคะแนนจับคู่งาน (Job Matching) ได้อย่างอัตโนมัติและมีประสิทธิภาพสูงสุด

##

### 🎯 1.2 วัตถุประสงค์ของการวิจัย

1. เเพื่อพัฒนาแบบจำลองคอมพิวเตอร์วิทัศน์ด้วย YOLO ในการวิเคราะห์โครงสร้างจัดวางเอกสาร (Document Layout Analysis) และจำแนกพื้นที่องค์ประกอบหลักของเรซูเม่ที่มีความหลากหลายคอลัมน์ได้อย่างถูกต้อง
...
- **ด้านเทคโนโลยีคอมพิวเตอร์วิทัศน์ (Computer Vision Scope)**
  - ใช้แบบจำลองโครงข่ายประสาทเทียม YOLO ในการวิเคราะห์โครงสร้างเชิงพื้นที่ของเอกสาร (Document Layout Analysis) เพื่อจำแนกประเภทองค์ประกอบหลักและคำนวณพิกัดขอบเขต (Bounding Box) บนเรซูเม่
  - ใช้ Google Cloud Vision API เป็นกลไกหลักในกระบวนการสกัดข้อความอัตโนมัติ (Optical Character Recognition: OCR) จากพื้นที่เอกสารที่ผ่านการประมวลผลปรับปรุงสเกลภาพภาพถ่าย (Image Preprocessing)
...
1. ได้ระบบต้นแบบที่มีความแม่นยำสูงในการสกัดข้อมูลจากเรซูเม่สองภาษาที่มีการจัดวางซับซ้อน (Multi-column Layouts) ซึ่งช่วยลดข้อจำกัดของระบบสกัดข้อความแบบดั้งเดิมที่ไม่รับรู้บริบทเชิงพื้นที่
...
4. ได้เว็บแอปพลิเคชันต้นแบบที่บุคคลทั่วไปหรือฝ่ายบริหารทรัพยากรมนุษย์สามารถเข้าถึง และใช้งานระบบสกัดข้อมูลพร้อมจับคู่งานได้จริงผ่านเครือข่ายอินเทอร์เน็ต
...
### 2.1 สถาปัตยกรรมรวมของระบบ (Overall System Architecture)
...
### 2.3 การวิเคราะห์และแยกแยะโครงสร้างเอกสารเชิงพื้นที่ (Document Layout Analysis using YOLO)
...
- **การออกแบบส่วนประสานงานและการติดตั้งใช้งานระบบ (User Interface & Deployment):** เพื่อให้ระบบสามารถนำไปประยุกต์ใช้งานได้จริงในอุตสาหกรรมการสรรหาบุคลากร งานวิจัยนี้จึงได้นำไปป์ไลน์การสกัดข้อความและการจับคู่งานทั้งหมด ไปออกแบบตัวประสานงานผู้ใช้และทำการติดตั้งระบบใช้งานจริง (Deployment) ในรูปแบบเว็บแอปพลิเคชันต้นแบบ (Web Application Prototype) บนแพลตฟอร์ม Hugging Face Spaces รองรับการอัปโหลดเอกสารเรซูเม่และการประมวลผลคำนวณคะแนนจับคู่งานแบบทันที (Real-time)
...
จากผล Confusion Matrix พบว่าภาพรวมของโมเดลสามารถแยกแยะคลาสที่มีอัตลักษณ์เฉพาะตัวสูง เช่น Education และ Skills ได้อย่างแม่นยำเด่นชัด ทว่าเมื่อพิจารณาเปรียบเทียบในแง่ของความสมดุล (Optimization) ระหว่างค่าความถูกต้องเชิงมิติและความเร็วในการประมวลผล (Inference Speed) คณะผู้จัดทำได้ตัดสินใจคัดเลือกโมเดลขนาด YOLO11s (Small) มาใช้งานเป็นตัวแบบหลัก เนื่องจากผลลัพธ์จากเมทริกซ์ความสับสนชี้ให้เห็นว่าให้อัตราความผิดพลาดต่ำใกล้เคียงกับโมเดลขนาดใหญ่ (Medium/X-Large) แต่ใช้ทรัพยากรระบบและเวลาประมวลผลต่อภาพต่ำกว่ามาก ซึ่งจำเป็นต่อการนำไปใช้งานจริงบนเว็บแอปพลิเคชันแบบทันที (Real-time)
...
หลังจากระบุเลือกตัวแบบ YOLO11s แล้ว คณะผู้จัดทำได้ทำการวิเคราะห์พฤติกรรมการเรียนรู้เชิงลึกตลอดยุคการฝึกฝน (Training Epochs) โดยพิจารณาจากเส้นกราฟการเรียนรู้ Train/Val Loss Curves ร่วมกับ F1-Confidence Curve และ Precision-Recall Curve เฉพาะของโมเดล YOLO11s พบว่า ค่าความสูญเสีย (Loss) ทั้งในชุดฝึกฝนและชุดทดสอบความถูกต้องมีแนวโน้มลดลงอย่างต่อเนื่องและลู่เข้าหากันอย่างมีเสถียรภาพ ไม่ปรากฏสัญญาณของ Overfitting ขณะที่พื้นที่ใต้กราฟความแม่นยำเฉลี่ย (mAP) พุ่งสูงขึ้นและคงที่ในระดับสูง พิสูจน์ได้ว่าโมเดล YOLO11s ที่คัดเลือกมีความพร้อมสูงในการทำงานร่วมกับข้อมูลที่ไม่เคยพบมาก่อน
...
จากการนำโมเดล YOLO11s ไปรันทำนายผลจริง (Inference) กับเอกสารเรซูเม่ชุดทดสอบ (Test Set) พบว่าระบบสามารถตีกรอบพิกัด (Bounding Box) พร้อมระบุป้ายกำกับคลาสส่วนประกอบหลักทั้ง 6 คลาส ได้อย่างครบถ้วนและแม่นยำสูง แม้จะเป็นเอกสารที่มีการจัดวาง Layout ซับซ้อนแบบหลายคอลัมน์ ระบบก็สามารถแยกแยะขอบเขตเชิงพื้นที่ออกจากกันได้ โดยไม่มีปัญหาข้อความข้ามช่องปนเปกัน ผลลัพธ์พิกัด Bounding Box ที่สะอาดจากโมเดล YOLO11s นี้ จึงทำหน้าที่เป็นสารตั้งต้นในการส่งต่อภาพตัดขอบเขต (Crop Image) ไปสู่กระบวนการสกัดข้อความ (OCR) ในขั้นตอนถัดไป

##

### 3.2.4 ผลการประเมินประสิทธิภาพรายคลาส (Per-Class Results)
เพื่อวัดความสามารถในการระบุขอบเขตพิกัดและการจำแนกประเภทส่วนประกอบ (Section) ทั้ง 6 คลาส บนชุดข้อมูลทดสอบ (Test Set) โดยใช้ตัวชี้วัดมาตรฐานสากลในงานตรวจจับวัตถุ (Object Detection) ได้แก่ ค่าความแม่นยำ (Precision) ค่าความระลึก (Recall) และค่าเฉลี่ยความแม่นยำรวมที่สัดส่วนความซ้อนทับร้อยละ 50 (mAP@0.5) ซึ่งผลสัมฤทธิ์เชิงสถิติแสดงดังตาราง
...
จากตารางผลการประเมินชี้ให้เห็นว่าโมเดล YOLO11s สามารถตรวจจับและจำแนก Layout ของเอกสารที่มีความหลากหลายเชิงโครงสร้างได้อย่างมีประสิทธิภาพสูง โดยมีค่า mAP@0.5 ในภาพรวมเฉลี่ยเท่ากับ 76.3% ซึ่งจัดอยู่ในเกณฑ์ที่เหมาะสมและน่าเชื่อถือสำหรับงานวิเคราะห์โครงสร้างจัดวางเอกสาร (Document Layout Analysis)

##

### 3.2.5 ผลจากการปรับแต่งโมเดลเชิงลึกเพื่อเพิ่มความแม่นยำในคลาสทักษะ (Fine-tuning Results for Skill Enhancement)
...
หลังจากที่แบบจำลอง YOLO11s ทำการวิเคราะห์และส่งมอบพิกัดขอบเขต (Bounding Box) ของแต่ละองค์ประกอบเอกสารเรียบร้อยแล้ว ภาพส่วนคัตเอาท์ (Crop Images) เหล่านั้นจะถูกนำเข้าสู่กระบวนการแปลงรหัสภาพให้กลายเป็นข้อความดิบ (Raw Text)

เพื่อคัดเลือกกลไกการสกัดข้อความที่มีความถูกต้องสูงสุด คณะผู้จัดทำได้ทำการทดลองเปรียบเทียบประสิทธิภาพของกลไกประมวลผลอักขระอัตโนมัติ (OCR Engines) จำนวน 4 ตัวเลือกหลัก ได้แก่ PaddleOCR, Surya OCR, EasyOCR และ Google Cloud Vision API โดยนำชุดภาพโครงสร้างเรซูเม่ที่สกัดได้ไปทดสอบประมวลผลภายใต้สภาพแวดล้อมการควบคุมเดียวกัน ผลสัมฤทธิ์เชิงคุณลักษณะและการจัดการภาษาแสดงดังนี้
...
จากตารางพบว่า Google Cloud Vision API เป็นกลไกที่ให้ประสิทธิภาพสูงสุดในทุกมิติ โดยเฉพาะอย่างยิ่งในการรับรู้และสกัดอักขระภาษาไทย ซึ่งเป็นข้อจำกัดของโมเดลรูปแบบออฟไลน์ (เช่น EasyOCR หรือ PaddleOCR) ที่มักจะตัดคำผิดพลาด แต่ต้องมี API Key โดยใช้งานได้ฟรี 1,000 รูป/เดือน
...
- ฟังก์ชันการเรียกใช้บริการตรวจจับข้อความสองภาษาแบบระบุคำสั่งอ้างอิง หลังจากระบุโครงสร้างรูปภาพพร้อมใช้แล้ว ระบบจะทำการเรียกฟังก์ชันหลักของตัวประสานงานผ่านอินเทอร์เน็ต (API Client) คือฟังก์ชัน client.text_detection() โดยมีการกำหนดค่าพารามิเตอร์เพื่อควบคุมความแม่นยำ 2 ส่วน คือ
  - กำหนดคุณลักษณะการตรวจจับเป็นแบบประเภทตรวจจับตัวอักษรทั่วไป (TEXT_DETECTION)
  - กำหนดพารามิเตอร์สำหรับการอ้างอิงแบบสองภาษาในโครงสร้างอินพุต หรือ language_hints=['th', 'en'] เพื่อบังคับและกำหนดกรอบให้ระบบ AI คลาวด์รับรู้และจำแนกอักขระภาษาไทยและภาษาอังกฤษที่วางผสมปนเปกันอยู่บนเอกสารเรซูเม่ได้อย่างแม่นยำสูงสุด

- กลไกการสกัดและดึงผลลัพธ์ข้อความดิบกลับสู่ระบบ (Response Parsing) เมื่อระบบคลาวด์ของ Google ทำการประมวลผลเสร็จสิ้นและส่งข้อมูลกลับมา (JSON API Response) ไปป์ไลน์จะเข้าถึงโครงสร้างตัวแปรเพื่อเจาะจงแกะเอาเฉพาะข้อมูลข้อความรวมทั้งหมดผ่านมางการอ้างอิงอาร์เรย์ชุดแรก เช่น response.text_annotations[0].description ซึ่งลอจิกในส่วนนี้จะทำหน้าที่สกัดเอาเฉพาะเนื้อหาข้อความดิบรวม (raw_text) ออกมาเป็นตัวแปรประเภทสายอักขระ (String) ก้อนเดี่ยว และทำการตัดข้อมูลเชิงมิติพิกเซลอื่น ๆ (Metadata) ที่ไม่จำเป็นทิ้งไป เพื่อส่งมอบข้อความดิบชุดนี้ข้ามฝั่งไปประมวลผลต่อในเฟส Bilingual NLP Pipeline ในหัวข้อถัดไป
...
เนื่องจากข้อความดิบที่สกัดได้จากเรซูเม่มีทั้งภาษาไทยและภาษาอังกฤษปะปนกัน ระบบจึงต้องระบุภาษาเพื่อเลือกไปป์ไลน์ในการทำความสะอาดข้อมูลที่เหมาะสม โดยการใช้เงื่อนไขตรวจสอบรหัสอักขระยูนิโค้ดอ้างอิงภาษาไทยผ่านคำสั่งคำนวณเชิงตรรกะ any('\u0e00' <= c <= '\u0e7f' for c in text)
หากตัวแปรตรวจสอบพบอักขระภาษาไทยแม้เพียงบางส่วน ระบบจะจำแนกข้อความกล่องนั้นให้อยู่ในกลุ่มภาษาไทยโดยอัตโนมัติเพื่อส่งเข้าฟังก์ชัน postprocess_thai() แต่หากไม่พบโครงสร้างรหัสยูนิโค้ดดังกล่าว ระบบจะส่งข้อความเข้าสู่วิธีการประมวลผลภาษาอังกฤษในฟังก์ชัน postprocess_en() ตามลำดับ
...
เมื่อข้อมูลถูกคัดแยกเป็นชุดข้อความภาษาไทย ปัญหาที่มักเกิดจากระบบ OCR คือการตรวจจับอักขระขยะ หรือโครงสร้างภาษาไทยที่ผิดเพี้ยน เช่น สระบน-ล่างซ้อนกัน หรือวรรณยุกต์ลอยจนระบบตัดคำวิเคราะห์ความหมายไม่ได้ ฟังก์ชัน postprocess_thai() จึงดำเนินการเรียกใช้คำสั่งปรับบรรทัดฐานตัวอักษรระดับสูง คือ ฟังก์ชัน th_normalize() จากไลบรารี PyThaiNLP เพื่อทำความสะอาดสระซ้อนและปรับระดับวรรณยุกต์ให้กลับมาถูกต้องตามมาตรฐานไวยากรณ์ พร้อมทั้งตัดช่องว่างส่วนเกินเพื่อให้เป็นสายอักขระที่พร้อมนำไปใช้งาน
...
ในขั้นตอนสุดท้ายของไปป์ไลน์ภาษาธรรมชาติ ข้อมูลที่ผ่านการปรับบรรทัดฐานและการลดรูปรากศัพท์เรียบร้อยแล้ว จะถูกนำมาสกัดขจัดช่องว่างและสัญลักษณ์ขยะส่วนเกินด้วยฟังก์ชันคำสั่งจัดกลุ่มตัวอักษร ' '.join(text.split()) เพื่อส่งคืนผลลัพธ์เป็นข้อความสะอาด (clean_text) จับคู่ผูกโยงเข้ากับป้ายกำกับของโมเดล YOLO11s และจัดเก็บรวบรวมลงในหน่วยความจำเชิงโครงสร้าง JSON Object ที่พร้อมส่งต่อไปประมวลผลคำนวณคะแนนในกลไกการจับคู่งาน (Job Matching Scoring Module) ร่วมกับฐานข้อมูลอ้างอิงเป็นสเต็ปสุดท้าย
...
เพื่อให้กลไกการประเมินคะแนนจับคู่งานมีความเที่ยงตรง คณะผู้จัดทำได้สถาปนาระบบวิศวกรรมข้อมูลเพื่อสร้างคลังคำศัพท์อ้างอิงกลาง (Vocabulary Base) โดยมีรายละเอียดและขั้นตอนลอจิกการคำนวณดังนี้
...
## 4.2 กลไกการประเมินคะแนนด้วยดัชนี (Scoring Matrix & Weighting Mechanism)
...
ในการประมวลผลขั้นสุดท้าย กลไกจับคู่งานจะนำค่า Importance Level และค่า Market_Freq มาผ่านฟังก์ชันคำนวณสัดส่วนค่าน้ำหนักภายในกลุ่มอาชีพ (Job_Group) นั้น ๆ ส่งผลให้ดัชนีคะแนนจับคู่งาน (Job Matching Score) ที่แสดงผลบนระบบประสานงานผู้ใช้ (User Interface) มีความเฉลี่ยที่แม่นยำ สมจริง และตอบโจทย์ความต้องการของตลาดแรงงานปัจจุบันได้อย่างดีที่สุด

---

## 5. การพัฒนาระบบ Job Matching และทดสอบระบบประสานงานผู้ใช้บนเว็บแอปพลิเคชัน (Web Application Implementation & System Integration) 
ในเฟสสุดท้ายของการดำเนินงาน คณะผู้จัดทำได้นำผลลัพธ์ข้อมูลเชิงโครงสร้างที่สกัดได้จากระบบส่วนหน้า มาทำการบูรณาการร่วมกับเฟรมเวิร์ก Streamlit เพื่อพัฒนาเป็นระบบแอปพลิเคชันต้นแบบจับคู่งาน (Job Matching Web Application Prototype) และนำขึ้นติดตั้งให้บริการจริงบนโครงสร้างพื้นฐานคลาวด์ของ Hugging Face Spaces โดยเน้นการรายงานผลสัมฤทธิ์ในมิติเชิงระบบและฟังก์ชันซอฟต์แวร์ดังต่อไปนี้

### 5.1 ผลการจัดสถาปัตยกรรมความปลอดภัยและการบริหารคลังข้อมูลคลาวด์ (Cloud Infrastructure & PDPA Implementation)
...
### 5.2 ฟังก์ชันการแสดงผลบนหน้าอินเทอร์เฟซแยกตามแถบประมวลผล (UI/UX Functional Tabs Showcase)
...
- แท็บ Resume (📸 ส่วนการตรวจสอบพิกัด): จัดแสดงผลลัพธ์เชิงภาพผ่านองค์ประกอบกล่องข้อความทับซ้อน (Overlay Bounding Box) แยกระดับสีตามคลาสโครงสร้าง (Section Colors) เพื่อให้ผู้ใช้งานตรวจสอบความถูกต้องเชิงตำแหน่งของระบบวิชันได้ทันที
- แท็บ Extracted (📋 ส่วนการจัดระเบียบคำสำคัญ): แสดงรายการคีย์เวิร์ดที่ผ่านการล้างและสกัดหมวดหมู่เรียบร้อยแล้ว โดยแยกกลุ่มข้อมูลเป็นตารางชิปไอคอน (Skill Chips) สำหรับทักษะเชิงเทคนิค และแสดงรายการทักษะทางสังคม (Soft Skills) ประสบการณ์ และประวัติการศึกษาอย่างชัดเจน
- แท็บ Top Groups & Top Jobs (🏆 ส่วนการสรุปกลุ่มตำแหน่ง): แสดงผลลัพธ์การจัดอันดับในรูปแบบกราฟแท่งเปรียบเทียบเปอร์เซ็นต์ความสอดคล้อง (fit_pct) ของผู้สมัครแยกตามกลุ่มสายอาชีพหลัก 27 กลุ่ม พร้อมทั้งเปิดกล่องขยาย (Expander) เพื่อแสดงรายชื่อตำแหน่งงานเฉพาะทางที่ผู้สมัครมีคุณสมบัติเหมาะสมสูงสุดตามลำดับ
- แท็บ Match (🔍 ส่วนการตรวจสอบย้อนกลับ): แสดงตารางแจกแจงพฤติกรรมการจับคู่ของคำศัพท์ทักษะในเรซูเม่เทียบกับคลังข้อมูลอ้างอิง เพื่อแสดงให้ผู้ใช้งานเห็นว่าทักษะตัวใดถูกนำไปประเมินและคิดเป็นค่าน้ำหนักคะแนนเท่าใด เพื่อสร้างความโปร่งใสและน่าเชื่อถือให้แก่ระบบซอฟต์แวร์
...
เมื่อผู้ใช้งานทำการเลือกตำแหน่งงานหรือกลุ่มอาชีพที่ต้องการ ระบบจะทำการดึงคะแนนของแคนดิเดตทุกคนมาประมวลผลเปรียบเทียบร่วมกันขนานไปพร้อมกัน จากนั้นจะนำผู้ที่มีคะแนนสูงสุด 3 อันดับแรกมาจัดแสดงผลบน Top 3 Podium Showcase พร้อมกราฟแท่งเปรียบเทียบภาพรวม และในขั้นตอนสุดท้าย ระบบได้จัดเตรียมปุ่มคำสั่งสำหรับดาวน์โหลดรายงานผลสัมฤทธิ์การจัดอันดับรวมออกมาเป็นโครงสร้างสากลในรูปแบบ JSON File ซึ่งเพิ่มความสะดวกรวดเร็วในการนำชุดข้อมูลสะอาดนี้ไปเชื่อมโยงร่วมกับระบบซอฟต์แวร์อื่น ๆ (เช่น ระบบ ATS หรือระบบฐานข้อมูลต่าง ๆ) ได้
